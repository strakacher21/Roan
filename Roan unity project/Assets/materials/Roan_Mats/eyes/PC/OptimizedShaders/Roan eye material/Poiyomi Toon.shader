Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/4f7326912ec552b4b8d9e50558f571ec"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 _PPTint;
			float3 _PPRGB;
			float _PPHue_Roaneyematerial;
			float _PPContrast;
			float _PPSaturation_Roaneyematerial;
			float _PPBrightness;
			float _PPLightness;
			float _PPHDR;
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue_Roaneyematerial, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, _PPSaturation_Roaneyematerial);
