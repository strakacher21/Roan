Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/bccec66fecd9f114698a2a758a1024e6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 _PPTint;
			float3 _PPRGB;
			float _PPHue_Roanheadmaterial;
			float _PPContrast;
			float _PPSaturation_Roanheadmaterial;
			float _PPBrightness;
			float _PPLightness;
			float _PPHDR;
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue_Roanheadmaterial, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, _PPSaturation_Roanheadmaterial);
