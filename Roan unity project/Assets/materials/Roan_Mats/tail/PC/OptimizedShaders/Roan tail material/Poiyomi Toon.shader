Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/78a7d2f555fbb87418faff5a29cd6c32"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 _PPTint;
			float3 _PPRGB;
			float _PPHue_Roantailmaterial;
			float _PPContrast;
			float _PPSaturation_Roantailmaterial;
			float _PPBrightness;
			float _PPLightness;
			float _PPHDR;
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue_Roantailmaterial, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, _PPSaturation_Roantailmaterial);
