Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/f15d75481ce245a4980f355484b57e3d"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateGlobalThemes(inout PoiMods poiMods)
			{
				...
				DecomposeHDRColor(float4(1,0,0,1).rgb, themeColor0.rgb, themeColorExposures.x);
				DecomposeHDRColor(float4(0,1,0,1).rgb, themeColor1.rgb, themeColorExposures.y);
				DecomposeHDRColor(float4(0,0,1,1).rgb, themeColor2.rgb, themeColorExposures.z);
				DecomposeHDRColor(float4(1,1,1,1).rgb, themeColor3.rgb, themeColorExposures.w);
				poiMods.globalColorTheme[0] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor0.rgb, frac(0.0 + 0.0 * _Time.x), 0.0, 0.0), themeColorExposures.x), float4(1,0,0,1).a);
				poiMods.globalColorTheme[1] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor1.rgb, frac(0.0 + 0.0 * _Time.x), 0.0, 0.0), themeColorExposures.y), float4(0,1,0,1).a);
				poiMods.globalColorTheme[2] = float4(ApplyHDRExposure(ModifyViaHSV(themeColor2.rgb, frac(0.0 + 0.0 * _Time.x), 0.0, 0.0), themeColorExposures.z), float4(0,0,1,1).a);
			...
			void ApplyGlobalMaskVertexColors(in PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (2 > 0)
				{
					applyToGlobalMask(poiMods, 2 - 1, 2, vcol.g);
				}
				if (3 > 0)
				{
					applyToGlobalMask(poiMods, 3 - 1, 2, vcol.b);
				}
				if (0.0 > 0)
				{
			...
			void calculateRGBNormals(inout PoiMesh poiMesh, inout PoiMods poiMods)
			{
				...
				if (0.0 > 0 && 1.0)
				{
					nR = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_RgbNormalR, _trilinear_repeat, poiUV(poiMesh.uv[0.0], PoiUVMerge(float4(1,1,0,0), float4(1,1,0,0) )), float4(0,0,0,0), 0.0), 0.0);
					hasR = 1;
			...
			void calculateRGBMask(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float4 red = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, 1.0), float4(1,1,1,1).a);
				float4 green = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, 2.0), float4(1,1,1,1).a);
				float4 blue = float4(poiThemeColor(poiMods, float4(1,1,1,1).rgb, 3.0), float4(1,1,1,1).a);
				...
				#endif
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[0.0], 1.0, 2.0, poiMods), red, 0.0, 8.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[1.0], 2.0, 2.0, poiMods), green, 0.0, 8.0, 0.0, 1.0);
				RGBABlendColor(poiFragData, globalMaskBlend(rgbMask[2.0], 3.0, 2.0, poiMods), blue, 0.0, 8.0, 0.0, 1.0);
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (1.0)
				{
					lns.x = poiEdgeNonLinearNoSaturate(lightMap.x, 0.875, 0.475 * blurMap.r);
					...
					lns.w = poiEdgeNonLinearNoSaturate(lightMap.x, 0.875, 0.475 * blurMap.r, 0.0);
				}
				else
				{
					lns.x = poiEdgeLinearNoSaturate(lightMap.x, 0.875, 0.475 * blurMap.r);
					lns.y = poiEdgeLinearNoSaturate(lightMap.y, 0.5, 0.3 * blurMap.g);
					lns.z = poiEdgeLinearNoSaturate(lightMap.z, 0.25, 0.1 * blurMap.b);
					lns.w = poiEdgeLinearNoSaturate(lightMap.x, 0.875, 0.475 * blurMap.r, 0.0);
				}
				#if defined(PROP_SHADOWBORDERMASK)
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
