Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/35ab7891782cfbb4f84292e7ca309c6f"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 _PPTint;
			float3 _PPRGB;
			float _PPHue_Roanfootmaterial;
			float _PPContrast;
			float _PPSaturation_Roanfootmaterial;
			float _PPBrightness;
			float _PPLightness;
			float _PPHDR;
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue_Roanfootmaterial, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, _PPSaturation_Roanfootmaterial);
