Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/980de9b2347ae1e40ab25548b3fafaf1"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 _PPTint;
			float3 _PPRGB;
			float _PPHue_Roanpantsmaterial;
			float _PPContrast;
			float _PPSaturation_Roanpantsmaterial;
			float _PPBrightness;
			float _PPLightness;
			float _PPHDR;
			...
			void applyPostProcessing(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				col = hueShift(col, _PPHue_Roanpantsmaterial, 1.0, 1.0);
				col *= float4(1,1,1,1);
				col *= float4(1,1,1,1);
				col = GetSaturation(col, _PPSaturation_Roanpantsmaterial);
